...
| Skills | Select one primary skill first, name why it triggered, inspect bounded source evidence, then report files read and approximate lines inspected. |
...
Without PDG, an agent can say it used a skill after reading only the skill prose. With PDG, risky skill calls must name the selected skill, bound the sources to inspect, and end with a read receipt: files read, approximate lines inspected, cited-but-unread files, and claims left `Unknown`.
